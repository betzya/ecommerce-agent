...
这一版代码只验证一件事：电商客服 Agent 能在本地终端把一组 `messages` 交给模型，并从模型响应里取出 `assistant message`。
...
```text
code/agent-course-versions/course.env.example
```

## 本地运行

```bash
cd code/agent-course-versions/lesson-01-model-messages
python main.py
```
