...
这一版代码让 `/chat` 后端第一次用“电商客服 Agent”的身份回答真实业务问题。它能把会员活动、订单物流、退款条件这类问题交给大模型生成自然语言回答，但它还没有任何可验证的电商业务事实来源。
...
课程里的每个 `code/agent-course-versions/lesson-xx-*/backend` 都只是一版 Agent 后端。电商客服 Agent 调试后台是根目录 `code/frontend/` 下的独立工具，通过目标 Agent 地址调用当前课程版本。
...
```bash
cd code/agent-course-versions/lesson-03-llm-customer-boundary/backend
python main.py
```
...
- `agents/`：新增“客服边界”编排，把电商身份、不能编业务事实等边界放进回答链路。
- `models/`：继续封装真实模型调用。
...
发送 `我是金卡会员，今天买降噪耳机能叠加优惠券吗？` 后，聊天区重点看第一版 AI 客服能用自然语言接住问题，但会明确说当前无法查询具体优惠规则，说明本课还没有接入可验证的电商业务事实来源：
