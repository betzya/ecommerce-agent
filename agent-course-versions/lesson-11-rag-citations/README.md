...
这一版代码在第 10 课“向量检索 + 模型回答 + 成本观察”的基础上增加 `citations`。电商客服 Agent 回答知识类问题时，仍然把向量命中片段交给模型生成回答，同时把命中的知识来源作为 `citations` 返回。
...
```bash
cd code/agent-course-versions/lesson-11-rag-citations/backend
python main.py
```
