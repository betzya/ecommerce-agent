...
embedding 配置仍然统一读取 `code/agent-course-versions/course.env`：
...
```bash
cd code/agent-course-versions/lesson-12-rag-quality-fallback/backend
python main.py
```
...
这张图看用户侧回答：没有足够可靠的电商规则依据时，Agent 会要求补充信息或建议转人工，而不是硬编结论。
