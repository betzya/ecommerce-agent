"""第 14 课 RAG Prompt 和 citation 构造。"""
...
def render_rag_messages(request: ChatRequest, rewrite: QueryRewrite, hits: list[KnowledgeHit]) -> list[dict[str, str]]:
    """把 reranker 重排后的知识渲染成模型输入。"""

    evidence = "\n".join(
        f"[{index}] {hit.chunk.title} score={hit.score} reasons={','.join(hit.rerank_reasons)}\n{hit.chunk.text}"
        for index, hit in enumerate(hits, start=1)
    )
    return [
        {
            "role": "system",
            "content": (
                "你是电商公司的客服 Agent。只能依据 reranker 最终排序后的知识回答；"
