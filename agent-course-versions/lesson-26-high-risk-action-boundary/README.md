...
这一版把用户的“直接给我退”挡在高风险边界上：Agent 可以从电商后端和运行时上下文查订单、查物流、检索售后政策并判断是否可发起申请，但不能执行退款、取消订单或补偿。
...
```bash
cd code/agent-course-versions/lesson-26-high-risk-action-boundary/backend
python main.py
```
