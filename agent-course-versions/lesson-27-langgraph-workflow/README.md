...
这一版把高风险售后从自由 Agent 路径里拆出来，用 LangGraph `StateGraph` 固定节点顺序，并在节点里读取电商后端和运行时上下文里的真实订单、物流事实。
...
```bash
cd code/agent-course-versions/lesson-27-langgraph-workflow/backend
python main.py
```
