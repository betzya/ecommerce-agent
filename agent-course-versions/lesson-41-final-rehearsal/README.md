...
- `tools/`、`integrations/`：订单详情、物流查询和电商业务后端集成，保证实时事实不靠模型猜。
- `rag/`：售后、发票、活动和会员规则的可引用知识片段；活动/会员规则会先形成候选池，再用课程版轻量 reranker 重排，保证第 14 课讲过的“候选池 -> 重排 -> citation”在总演习里闭环。
...
`/eval/run` 会同时检查答案、Tool、RAG citation、Trace、session_state 和 HITL 恢复边界。本次总演习 14 个 case 全部通过：原有 9 条大促主线保持不变，新增 5 条用于防止否定退款误路由、商品联合回答、退款进度、已发货退款和签收后退货能力倒退。
...
```bash
cd code/agent-course-versions/lesson-41-final-rehearsal/backend
python main.py
```

启动后端后，可以用 `/eval/run` 触发本课 14 条大促回归场景：

```bash
curl -s -X POST http://127.0.0.1:8000/eval/run
```

返回里的 `passed` 应为 14，`failed` 应为 0。这里检查的是 answer、Tool、RAG citation、Trace、session_state，以及 3 条审批恢复 / resume 场景，不只是最终话术。
