"""客服 Agent 编排层。它复用 Tool、RAG、Workflow、Trace、Eval 和成本治理能力。"""
...
from observability.metrics import DEGRADED_TOTAL
...
from state.db import increment_message_count
...
class CustomerServiceAgent:
    ...
    def chat(self, request: ChatRequest) -> ChatResponse:
        """编排综合链路聊天链路，串联 Tool、RAG、Workflow/HITL、降级、安全、Trace 和成本治理。"""
        load_env()
        message_count = increment_message_count(request.session_id)
        ...
        route_was_guarded = str(route_result.fallback_reason or "").startswith(
            ("rule_guard_", "rule_veto_", "rule_promote_")
        )
        ...
        if uses_runtime_identity:
            ...
            DEGRADED_TOTAL.labels(reason=degradation_reason).inc()
            degradation_source = "user_reported_service_failure"
            trace_store.add(
                request.session_id,
        ...
        return ChatResponse(
            ...
            reasoning_summary=[
                ...
                "reasoning_content 不对外返回；系统提示词、密钥、隐私原文和内部堆栈不会写入公开 trace。",
            ],
            ...
            session_state={
                ...
                "next_gap": "用同一阶段 Agent 做综合链路，并把证据整理成项目表达。",
            },
        )
    ...
    def _apply_route_guard(
        *,
        ...
        # 小模型误判为 general_chat，但规则明确识别出具体意图时，用规则结果覆盖（仅低风险意图）
        if route_result.intent == "general_chat":
            rule_intent = classify_intent(user_message)
            if rule_intent not in {"general_chat", "unknown"}:
                return ModelRouteResult(
                    intent=rule_intent,
                    candidate=route_result.candidate,
                    used_model=route_result.used_model,
                    model_name=route_result.model_name,
                    fallback_reason=f"rule_promote_{rule_intent}",
                    framework=route_result.framework,
                    prompt_fragments=route_result.prompt_fragments,
                )
