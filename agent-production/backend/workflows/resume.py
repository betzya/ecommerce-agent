...
from state.db import get_checkpoint, get_submitted_action, record_submitted_action, save_checkpoint
...
def resume_from_checkpoint(request: ChatResumeRequest, trace_store: Any) -> ChatResumeResponse:
    ...
    checkpoint = get_checkpoint(request.session_id, request.workflow_id)
    ...
    existing_action = get_submitted_action(idempotency_key)
    idempotent_replay = existing_action is not None
    if request.decision == "approved":
        workflow["status"] = "completed"
        workflow["pending_action"] = "approval_accepted"
        answer = "售后主管已批准模拟退款申请，系统记录审批通过；本服务不执行真实资金退款。"
        status: Literal["completed", "paused", "rejected", "blocked"] = "completed"
        if existing_action:
            request_id = existing_action["request_id"]
        else:
            request_id = f"refund-{request.workflow_id}"
            record_submitted_action(
                idempotency_key,
                request_id,
                request.workflow_id,
                checkpoint["frozen_fields"].get("order_id"),
            )
