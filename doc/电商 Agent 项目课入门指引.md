这份文档不是单独介绍“课程助手”的使用说明，而是帮你完成“电商 Agent 项目课”的入门准备：安装 Trae CN、获取课程代码、安装课程 Skill、确认后续运行手册在哪里查看，并知道第一次学习时应该怎么向 AI 助手提问。

你需要完成两件事：

+ 从云效/Codeup 拉取课程代码仓。
+ 在 Trae CN 中打开这个代码仓，并确认“课程助手”Skill 可以使用。

后续真正运行课程、配置环境、启动某一课、排查报错时，文档以语雀共享文件目录中的运行手册为准；课程助手可以帮你启动系统，也负责把运行手册、课程代码、每课 README、调试台信号和课程知识点串起来，帮你少走弯路。
...
| `ecommerce-backend/` | 电商公司的业务后端，包括订单、物流、退款、审批等接口 |
...
## 3. 安装课程助手 Skill
课程助手是项目课配套的 AI 导学 Skill，名称是：

```latex
$release-course-assistant
```
...
2. 进入 Trae 的 **设置**。
3. 找到 **技能与命令** 或 **技能** 页面。
4. 查看项目技能列表中是否有“课程助手”或 `release-course-assistant`。
...
3. 在 **技能** 部分点击 **创建**。
4. 优先选择 **项目技能**。
5. 上传 `release-course-assistant.zip`，或选择解压后的 `SKILL.md`。

> 📎 **Skill 安装包下载**：[release-course-assistant.zip](https://www.yuque.com/attachments/yuque/0/2026/zip/40749310/1783491668283-ebf9c439-8abc-4549-8b27-d04498b4ae0a.zip)
>
...
```latex
使用课程助手，我刚打开电商 Agent 项目课代码仓。请帮我检查当前目录结构，并告诉我下一步应该先看运行手册、课程 README，还是先配置环境。
```

如果回答中能识别 `agent-course-versions/`、`frontend/`、`ecommerce-backend/` 等目录，并提示你按照运行手册准备环境，就说明 Skill 基本可用。

## 4. 课程助手能帮你做什么
课程助手不是替代课程正文的“全文问答库”。它的定位是项目课导学和运行助手：把课程知识点、代码仓、运行手册、调试台信号连接起来。

### 4.1 检查代码仓和运行环境
你可以问：

```latex
使用课程助手，请检查当前目录是不是课程代码仓根目录。
```

```latex
使用课程助手，我的课程代码仓在 `/path/to/e-commerce`，请帮我检查运行第 20 课前还缺什么。
```
...
```latex
使用课程助手，Tool 调用是哪几课讲的？我应该从哪一课开始看？
```

```latex
使用课程助手，RAG 在这门课里是怎么展开的？
```

```latex
使用课程助手，工具结果怎么治理回上下文？应该看哪些课程和代码？
```
...
```latex
使用课程助手，帮我生成第 20 课的启动指引。
```

```latex
使用课程助手，我的课程代码仓在 `/path/to/agent-course-code`，请帮我检查环境并启动第 41 课。
```
...
```latex
使用课程助手，第 21 课和第 20 课相比新增了什么？
```

```latex
使用课程助手，第 35 课新增能力怎么在调试台观察？
```
...
```latex
使用课程助手，怎么看代码才能理解工具调用结果是怎么进入上下文的？
```

```latex
使用课程助手，第 30 课的 HITL 机制应该看哪些文件？
```

```latex
使用课程助手，Trace 面板的数据是从哪里来的？
```
...
```latex
使用课程助手，我启动第 18 课时报错了，错误信息如下：……
```

```latex
使用课程助手，页面打不开，前端端口是 5173，Agent 后端端口是 8000，请帮我排查。
```

```latex
使用课程助手，preflight 提示缺少 AGENT_OPENAI_API_KEY。请告诉我应该改哪个 course.env 文件，不要让我把 Key 发到聊天里。
```
...
课程助手可以辅助你理解和执行运行手册，但遇到具体环境安装步骤时，请以语雀共享文件目录中的运行手册为准。
...
4. 确认 `.trae/skills/release-course-assistant/` 或 `.agents/skills/release-course-assistant/` 是否存在。
5. 在 Trae 中确认课程助手是否可用。
6. 查看语雀共享文件目录中的运行手册。
7. 让课程助手检查当前代码仓和运行前提。
8. 再选择第一节要阅读或运行的课程。
...
```latex
使用课程助手，我刚拿到电商 Agent 项目课代码仓。请先帮我检查当前目录结构是否正确，再告诉我第一天应该先看哪些文件、哪些手册，以及是否需要马上启动服务。
```

如果你已经准备开始跑课程，可以问：

```latex
使用课程助手，我已经看过运行手册，想启动第 20 课。请帮我检查环境、确认 course.env 配置位置，并生成启动指引。
```

如果你还不知道从哪条能力线开始，可以问：

```latex
使用课程助手，请按能力线整理这门项目课：Tool、RAG、Prompt、上下文、Memory、LangGraph、HITL、Trace、Evaluation、成本治理分别在哪些课展开？
```

## 7. 常见问题
### Q1：为什么代码仓里没有完整课程正文？
云效/Codeup 代码仓主要发放可运行代码、调试台、场景材料和启动文件。完整课程正文不随代码仓发布，用来保护课程内容。学习时请结合正式课程内容、语雀共享文件目录和课程助手的精炼导读。

### Q2：代码仓里有两个 Skill 目录，应该用哪一个？
优先看 `.trae/skills/release-course-assistant/`，这是 Trae 项目技能目录。`.agents/skills/release-course-assistant/` 是 Agent Skills 生态常见目录，作为兼容预置。你只要能在 Trae 技能列表里看到课程助手，就可以正常使用。

### Q3：Skill 没有自动生效怎么办？
先确认当前打开的是课程代码仓根目录。然后进入 Trae 的技能列表，查看是否有“课程助手”或 `release-course-assistant`。如果没有，使用课程提供的 `release-course-assistant.zip` 手动导入为项目技能。

### Q4：我应该先看运行手册，还是先问课程助手？
第一次建议先知道运行手册在哪里，再问课程助手帮你检查目录和学习路线。真正安装环境、配置 Key、启动服务时，运行手册是权威步骤；课程助手适合帮你把步骤对应到当前代码仓和具体课程。
...
```latex
使用课程助手，我的课程代码仓在 `/path/to/agent-course-code`，我启动第 20 课后页面打不开，前端端口是 5173，Agent 后端端口是 8000，系统是 macOS。下面是终端输出：……
```

路径、课程编号、系统、端口、命令和完整错误信息越清楚，排查越快。

### Q7：Trae 自己出问题了怎么办？
如果是 Trae 安装、登录、窗口、插件市场、输入法、快捷键等问题，优先查看 [Trae 官方常规问题文档](https://docs.trae.cn/ide_troubleshoot-general-issues)。如果是电商 Agent 项目课运行问题，再使用课程助手和运行手册排查。
